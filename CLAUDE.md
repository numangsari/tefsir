...
## Son Güncelleme
2026-06-06 (8. iş, ek) — **İletişim yanıtı site adına gönderiliyor**. Sorun: panel "Yanıtla" sadece `mailto:` linkiydi → cevap admin'in kendi Gmail'inden gidiyordu (ziyaretçi tefsir.net'ten yanıt bekliyor). Çözüm: panelde satır içi yanıt kutusu + `POST /api/admin/contact/reply` → `sendContactReply` (Resend; `from: noreply@tefsir.net`, `to: ziyaretçi`, `replyTo: CONTACT_EMAIL`; gövdede yanıt + alıntılı orijinal mesaj). Gönderince mesaj okundu işaretlenir + audit `contact.reply`. Yeni DB kolonu YOK (mevcut ContactMessage). tsc/build temiz.
