...
## Son Güncelleme
2026-06-07 (11. iş) — **Açılır menü şeffaflık düzeltmesi.** Form input'ları ve açılır menüler (`TopBar` mobil menü, `SurahIndexView`/`PanelView`/`arama` input+select'leri) glassmorphism tasarımdan kaynaklı `/70`–`/90` opaklık alarak arka planı gösteriyordu. Hepsi tam opak (`bg-white dark:bg-stone-900/800`) yapıldı. Kart yüzeyleri (`surface-glass`) tasarım kararı olarak yarı saydam kaldı. ⚠️ Yeni form elemanı/menü eklerken `bg-white dark:bg-stone-9xx` (tam opak) kullan — `surface-glass` kart/panel içindir, dropdown/input için değil.
Önceki (10. iş) — **Efektli/modern tasarım yenilemesi (yalnız görsel, fonksiyon değişmedi)**. Hedef: kullanıcının paylaştığı koyu zeminli panel görselleri (emerald/teal radyal ışıma, cam/glassmorphism kart, parlayan rozet, pill+glow buton, zarif serif). Yaklaşım = **merkezi tasarım katmanı** (komponent bazlı değil): (1) `globals.css` → `body::before` ambient ışıma (koyu taban `#0a0a0a`; açıkta yumuşak) + `@layer components` sınıfları `.surface-glass`, `.surface-glass-hover`, `.btn-glow`, `.btn-outline-glow`, `.badge-glow` + `animate-fade-up`; okuma metni stilleri (`.hl/.note-inline/.arabic/.tefsir-body`) **birebir korundu**. (2) `tailwind.config.ts` → `fontFamily.serif = [var(--font-serif),…]`, `boxShadow.glow/glow-lg`, fade-up keyframe. (3) Serif **Newsreader** `<link>` ile runtime'da (Amiri gibi; `next/font` DEĞİL → build-time ağ bağımlılığı yok), `:root --font-serif`. **Tema:** her ikisi korundu — TopBar/landing/auth gibi eskiden hep koyu-yeşil olan yüzeyler artık her iki temada okunur (adaptif `dark:` renkler). **Okuyucu:** ölçülü — paneller cam, orta tefsir metni yüksek kontrastlı/sade. Sınıf swap'ı yapılan dosyalar: `TopBar`, `BrandLogo`, `page.tsx`, `AuthUnified`, `iletisim/ContactForm`, `AyahStickyHeader`, `TafsirReader`, `AnnotationTools`, `BookmarkButton`, `FontSizeControl`, `NotesPanel`, `SearchPalette`, `ScrollToTopButton`, `yonetici/ui.tsx` (Card/StatCard → tüm sekmeler), `profil`, `panel/PanelView`, `sureler/SurahIndexView`, `arama`. tsc+build temiz; dev'de `/`,`/oku/2/6`,`/giris`,`/iletisim` 200 + render doğrulandı. ⚠️ Yeni yüzey eklerken hazır `.surface-glass`/`.btn-glow` sınıflarını kullan (yeniden inline stil yazma). ⚠️ Kullanıcının gözle (iki tema + mobil) onayı kaldı.
Önceki (9. iş) — **Ana sayfa güncellemeleri + okuma ilerlemesi**. (1) Hero metni "…anlaşılır biçimde okuyun." olarak güncellendi; (2) hero'da BrandMark sembolünün sağına büyük "tefsir.net" yazısı; (3) TopBar hadis-i şerif fontu büyütüldü (text-xs→text-base serif); (4) TopBar arama ikonu büyütüldü (text-lg→text-2xl, w-9 h-9); (5) **Hesabım → Okuma ilerlemesi** bölümü: `GET /api/my/progress` (raw SQL — sûre bazında okunan TafsirReadMark / o sûredeki toplam TafsirContent; yalnız başlanan sûreler, yüzde desc sıralı), profilde her sûre için yüzde + ilerleme barı + sûreye link. Metrik kararı kullanıcıyla: **tam tefsir kapsamı** (payda=gerçek tefsir içeriği sayısı, ayetCount×11 değil → eksik içerikli ayetlerde %100 ulaşılabilir). Yeni DB kolonu YOK. tsc/build temiz.
